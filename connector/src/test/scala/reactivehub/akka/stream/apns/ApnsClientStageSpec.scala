...
class ApnsClientStageSpec(_system: ActorSystem)
  ...
  it should "accept a valid notification with known topic" in
    withServer { (stage, server) ⇒
      ...
      pub.sendNext(
        1 → Notification(ApnsHelpers.knownToken,
                         validPayload,
  ...
  it should "reject a notification with unknown token" in
    withServer { (stage, server) ⇒
      ...
      sub.requestNext(
        1 → Response.Failure(Sc.BadRequest, R.BadDeviceToken, None))
      sub.expectComplete()
    }
  ...
  it should "reject a notification with unknown topic" in
    withServer { (stage, server) ⇒
      ...
      sub.requestNext(
        1 → Response.Failure(Sc.BadRequest, R.TopicDisallowed, None))
      sub.expectComplete()
    }
    ...
    withServer { (stage, server) ⇒
      ...
      Await.result(
        Source
          ...
          .via(stage)
          .runWith(Sink.foreach(results += _._1)),
        10.seconds
      )
...
private[apns] object ApnsHelpers {
  final case class ReceivedRequest(method: Option[String],
                                   path: Option[String],
                                   id: Option[String],
  ...
  val knownToken =
    DeviceToken(
      "a7729a6af17775f80fdd8042b20e2a6f9fb1df2bc10f6bdb896ca41a30ba0455")

  val unregisteredToken =
    DeviceToken(
      "a7729a6af17775f80fdd8042b20e2a6f9fb1df2bc10f6bdb896ca41a30ba0456")

  val unknownToken =
    DeviceToken(
      "a7729a6af17775f80fdd8042b20e2a6f9fb1df2bc10f6bdb896ca41a30ba0457")
...
private[apns] trait ApnsHelpers extends NettyHelpers with BeforeAndAfterAll {
  ...
  final class ApnsServerConnection(channel: SocketChannel,
                                   probe: TestProbe,
                                   dataProbe: TestProbe) {
  ...
  final class ApnsServerMock(sslContext: SslContext, group: NioEventLoopGroup)
      extends TestableServer[ApnsServerConnection](group,
                                                   halfClose = false,
                                                   channelReadyWhenActive =
                                                     false) {
    ...
    override protected def createConnection(
      ...
      new ApnsServerConnection(channel.asInstanceOf[SocketChannel],
                               probe,
                               dataProbe)
  }

  private class Builder
      extends AbstractHttp2ConnectionHandlerBuilder[ApnsServerHandler, Builder] {
    override def build(): ApnsServerHandler = super.build()

    override def build(decoder: Http2ConnectionDecoder,
  ...
  private class ApnsServerHandler(decoder: Http2ConnectionDecoder,
                                  encoder: Http2ConnectionEncoder,
    ...
    private def sendOk(ctx: ChannelHandlerContext,
                       stream: Http2Stream,
    ...
    private def sendNok(ctx: ChannelHandlerContext,
                        stream: Http2Stream,
                        id: Option[String],
      ...
      val headers =
        new DefaultHttp2Headers().status(statusCode.intValue.toString)
      id.foreach(headers.set(HeaderId, _))
    ...
    private def sendGoAway(ctx: ChannelHandlerContext,
                           reason: Option[Reason]): Unit = {
      goAway(
        ctx,
        connection.remote.lastStreamCreated(),
        Http2Error.CANCEL.code(),
        reason
          .map(r ⇒ ByteBufUtil.writeUtf8(ctx.alloc(), writeReason(r)))
    ...
    private def writeReason(reason: Reason,
                            timestamp: Option[Long] = None): String = {
  ...
  def withServer(
      f: (ApnsClientStage[Int, NioSocketChannel], ApnsServerMock) ⇒ Any)(
      implicit ru: ResponseUnmarshaller): Unit = {
    val server = new ApnsServerMock(serverSslContext, serverGroup)
    try {
      val stage =
        NioApnsClientStage[Int](server.address, clientSslContext, clientGroup)
      f(stage, server)
    } finally {
      server.close()
    }
