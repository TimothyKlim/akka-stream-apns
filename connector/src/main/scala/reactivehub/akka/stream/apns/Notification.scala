...
/**
  ...
  */
final case class Notification(deviceToken: DeviceToken,
                              payload: Payload,
                              id: Option[UUID] = None,
...
object Notification {
  def apply(deviceToken: DeviceToken,
            payload: Payload,
...
object Payload {
  ...
  implicit def forBuilder[B, N, W[_]](b: B)(
      implicit ev: B ⇒ Builder[N, W]): Payload = b.result
  ...
  sealed trait Builder[N, W[_]] {
    ...
    def withCustomField[T](key: String, value: T)(
        implicit writer: W[T]): Builder[N, W]
    def result: Payload
  }
  ...
  object Builder {
    ...
    implicit def forString(alert: String)(
        implicit m: PayloadMarshaller): Builder[m.Node, m.Writer] =
      new SimpleBuilder[m.Node, m.Writer](m, alert = Some(alert))
  }

  private[apns] case class EmptyBuilder[N, W[_]](
      marshaller: PayloadMarshaller.Aux[N, W])
      extends Builder[N, W] {
    override def withAlert(alert: String): Builder[N, W] =
      SimpleBuilder[N, W](marshaller, alert = Some(alert))

    override def withLocalizedAlert(key: String,
                                    args: String*): Builder[N, W] =
    ...
    override def withLocalizedTitle(key: String,
                                    args: String*): Builder[N, W] =
      CompBuilder[N, W](marshaller,
                        titleLocKey = Some(key),
                        titleLocArgs = Some(args))
    ...
    override def withBadge(badge: Int): Builder[N, W] =
      SimpleBuilder[N, W](marshaller, badge = Some(badge))

    override def withSound(sound: String): Builder[N, W] =
      SimpleBuilder[N, W](marshaller, sound = Some(sound))

    override def withContentAvailable: Builder[N, W] =
      SimpleBuilder[N, W](marshaller, contentAvailable = Some(1))

    override def withCategory(category: String): Builder[N, W] =
      SimpleBuilder[N, W](marshaller, category = Some(category))

    override def withCustomField[T](key: String, value: T)(
        implicit writer: W[T]): Builder[N, W] =
      SimpleBuilder[N, W](marshaller,
                          customFields =
                            Map(key → marshaller.write(value, writer)))

    override def result: Payload = {
      val json =
        marshaller.jsonObject(Map("aps" → marshaller.jsonObject(Map.empty)))
  ...
  private[apns] case class SimpleBuilder[N, W[_]](
    ...
    override def withBadge(badge: Int): Builder[N, W] =
      copy(badge = Some(badge))

    override def withSound(sound: String): Builder[N, W] =
      copy(sound = Some(sound))

    override def withContentAvailable: Builder[N, W] =
      copy(contentAvailable = Some(1))

    override def withCategory(category: String): Builder[N, W] =
      copy(category = Some(category))

    override def withCustomField[T](key: String, value: T)(
        implicit writer: W[T]): Builder[N, W] =
      copy(
        customFields =
          customFields.updated(key, marshaller.write(value, writer)))
    ...
    override def result: Payload = {
      val aps = Map(
        ...
        "category" → category.map(marshaller.jsonString)
      ).filter(_._2.isDefined).mapValues(_.get)

      val json = marshaller.jsonObject(
        customFields ++ Map("aps" → marshaller.jsonObject(aps)))
      Payload(marshaller.print(json))
    }
  }
  ...
  private[apns] case class CompBuilder[N, W[_]](
    ...
    override def withLocalizedAction(key: String): Builder[N, W] =
      copy(actionLocKey = Some(key))

    override def withLaunchImage(launchImage: String): Builder[N, W] =
      copy(launchImage = Some(launchImage))
    ...
    override def result: Payload = {
      val alert = Map(
        ...
        "title-loc-args" → titleLocArgs.map(s ⇒
          marshaller.jsonArray(s.map(marshaller.jsonString))),
        "action-loc-key" → actionLocKey.map(marshaller.jsonString),
        "loc-key" → locKey.map(marshaller.jsonString),
        "loc-args" → locArgs.map(s ⇒
          marshaller.jsonArray(s.map(marshaller.jsonString))),
        "launch-image" → launchImage.map(marshaller.jsonString)
      ).filter(_._2.isDefined).mapValues(_.get)
...
object Expiration {
  implicit def apply(duration: Duration): Expiration = duration match {
    case d: FiniteDuration ⇒
      Expiration((System.nanoTime().nanos + d).toSeconds)
